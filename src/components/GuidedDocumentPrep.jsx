...
import { getDocumentGuide, buildGuidedPayload } from '../lib/documentGuides.js'
...
export default function GuidedDocumentPrep() {
  ...
  const guidedDraft = useDiagnosticStore((s) => s.guidedDraft ?? null)
  ...
  useEffect(() => {
    ...
    const incomingDraft =
      location.state?.guidedDraft?.guideId === guide.id
        ? location.state.guidedDraft.flatAnswers
        : guidedDraft?.guideId === guide.id
          ? guidedDraft.flatAnswers
          : null

    setAnswers(mergeAnswersWithGuide(guide, incomingDraft || {}))
    setAttemptedSubmit(false)
  }, [guide?.id, location.state, guidedDraft])
  ...
  return (
    <div className="p360-responsive-guided" style={{ padding: 24, alignItems: 'start' }}>
      <aside
        className="p360-card p360-aside-sticky"
        style={{
          padding: 18,
          display: 'flex',
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13.5, color: '#5F675F', lineHeight: 1.7 }}>
            This workflow guides you through the questions needed to prepare a strong {guide.title.toLowerCase()}. Your answers are structured and used to generate the final draft.
          </div>
        </div>
        ...
        >
          ...
          {guide.estimatedMinutes ? (
            <div style={{ fontSize: 11.5, color: '#8B938B', marginTop: 6 }}>
              Estimated time: {guide.estimatedMinutes} min
            </div>
          ) : null}
        </div>

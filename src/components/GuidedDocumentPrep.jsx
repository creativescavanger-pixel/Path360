...
function getSectionStats(section, answers) {
  ...
  const answered = questions.filter((q) => isQuestionAnswered(answers[q.key])).length
  const requiredTotal = questions.filter((q) => q.required).length
  const requiredAnswered = questions.filter((q) => q.required && isQuestionAnswered(answers[q.key])).length
...
function getOverallStats(guide, answers) {
  ...
  return {
    ...
    answered,
    completedSections,
    totalSections: guide?.sections?.length || 0,
    percent: total ? Math.round((answered / total) * 100) : 0,
  }
}
...
function clampSectionIndex(index, guide) {
  const max = Math.max((guide?.sections?.length || 1) - 1, 0)
...
export default function GuidedDocumentPrep() {
  ...
  useEffect(() => {
    ...
    async function loadDraft() {
      ...
      try {
        ...
        const savedSectionIndex = clampSectionIndex(Number(rawDraftData?.currentSectionIndex ?? 0), guide)
  ...
  const hasSections = Array.isArray(guide?.sections) && guide.sections.length > 0
  const currentSection = hasSections ? guide.sections[currentSectionIndex] : null
  ...
  const currentMissing = currentSection && attemptedNext
    ...
    : []
  const isLastSection = hasSections ? currentSectionIndex === guide.sections.length - 1 : false
  const allSectionsComplete = overall ? overall.completedSections === overall.totalSections && overall.totalSections > 0 : false
        ...
        draftData: {
          ...
          currentSectionIndex:
            typeof overrides.currentSectionIndex === 'number'
              ? clampSectionIndex(overrides.currentSectionIndex, guide)
              : currentSectionIndex,
  ...
  useEffect(() => {
    if (!guide || !user?.id || isHydratingRef.current) return
  ...
  }, [overall?.completedSections, lastSavedCompletedSections, currentSection, gamification.completedLabel])
  ...
  const handleSectionJump = async (index) => {
    const nextIndex = clampSectionIndex(index, guide)

...
import useDiagnosticStore, {
  saveLocalStage,
} from '../stores/useDiagnosticStore.js'
import {
...
const clearStageDraft = useDiagnosticStore(
  (state) => state.clearStageDraft,
)
...
useEffect(() => {
  ...
  setSelectedStage(draft.selectedStage || '')
  setStatusByItem({
    ...createEmptyStatuses(),
    ...(draft.statusByItem || {}),
  })
  setNotes(draft.notes || '')
  setProductMaturity(draft.productMaturity || '')
  setCustomerStatus(draft.customerStatus || '')
  setRevenuePattern(draft.revenuePattern || '')
  setGoToMarketRepeatability(draft.goToMarketRepeatability || '')
  setTeamStructure(draft.teamStructure || '')
  setFundingStage(draft.fundingStage || '')
  setPriorities(Array.isArray(draft.priorities) ? draft.priorities : [])
  setBlockers(Array.isArray(draft.blockers) ? draft.blockers : [])
  setCompletedActivities(
    Array.isArray(draft.completedActivities)
      ? draft.completedActivities
      : [],
  )
  setVentureSummary(draft.ventureSummary || '')
  setVentureProblem(draft.ventureProblem || '')
  setVentureProgressLevel(draft.ventureProgressLevel || '')
  setRecentProgress90Days(draft.recentProgress90Days || '')
  setMainTension(draft.mainTension || '')
  setPathDecision(draft.pathDecision || '')
  ...
  async function handleSaveAndContinue() {
    ...
    try {
      ...
      const nextAssessment = {
        declaredStage: selectedStage || recommendedStageId,
      ...
      {showPathways && (
        ...
        >
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="stage-onboarding-completion-title"
            style={{
              width: 'min(100%, 540px)',
              padding: 28,
              ...
              boxShadow: '0 22px 65px rgba(17, 24, 20, 0.24)',
            }}
          >
            <p
              style={{
            ...
            >
              Founder baseline complete
            </p>

            <h2
              id="stage-onboarding-completion-title"
              style={{
                margin: '0 0 12px',
            ...
            >
              <p style={{ margin: 0 }}>
                You now have a current starting stage and a recommended
                Path360 pathway. This is not a fixed label—it changes as your
                evidence, progress, and priorities change.
              </p>
